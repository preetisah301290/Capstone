from util import *
from constants import yogasutra_file, pickle_path, csv_path, roman_numeral,yogasutra_studyline, upnishad_file
import pickle
from collections import Counter

def yogasutra_preprocess():
    req = open(yogasutra_file)
    output_dict = {}
    ...
    of = open(pickle_path+"yogasutra.pickle", "wb")
    pickle.dump(final_dict, of)


def yogasutra_baseline():
    f = open(pickle_path+"yogasutra.pickle","rb")
    ...
    execute_similatity_matrix(DTM,  type="yogasutra", label="baseline", col_row_labels=row_labels)

    of = open(pickle_path+"yogasutra.pickle","wb")
    pickle.dump(input_dict, of)

def yogasutra_find_similarChp(Chapter_index):
    data = np.genfromtxt(csv_path+'yogasutra_baselineCosineSimilarity.csv',delimiter=',')
