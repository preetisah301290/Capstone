from util import *
from constants import taoteching_input_file, pickle_path, csv_path,BookEcclesiastes_file, BookEcclesiastes_studyline
import pickle
from collections import Counter

def BookEcclesiastes_preprocess():
    req = open(BookEcclesiastes_file)
    output_dict = {}
    ...
    of = open(pickle_path+"BookEcclesiastes.pickle", "wb")
    pickle.dump(final_dict, of)


def BookEcclesiastes_baseline():
    f = open(pickle_path+"BookEcclesiastes.pickle","rb")
    ...
    execute_similatity_matrix(DTM,  type="BookEcclesiastes", label="baseline", col_row_labels=row_labels)

    of = open(pickle_path+"BookEcclesiastes.pickle","wb")
    pickle.dump(input_dict, of)

def BookEcclesiastes_find_similarChp(Chapter_index):
    data = np.genfromtxt(csv_path+'BookEcclesiastes_baselineCosineSimilarity.csv',delimiter=',')
