from util import *
from constants import taoteching_input_file, pickle_path, csv_path,BookProverb_studyline, BookProverb_file
import pickle
from collections import Counter

def BookProverb_preprocess():
    req = open(BookProverb_file)
    output_dict = {}
    ...
    of = open(pickle_path+"BookProverb.pickle", "wb")
    pickle.dump(final_dict, of)


def BookProverb_baseline():
    f = open(pickle_path+"BookProverb.pickle","rb")
    ...
    execute_similatity_matrix(DTM,  type="BookProverb", label="baseline", col_row_labels=row_labels)

    of = open(pickle_path+"BookProverb.pickle","wb")
    pickle.dump(input_dict, of)

def BookProverb_find_similarChp(Chapter_index):
    data = np.genfromtxt(csv_path+'BookProverb_baselineCosineSimilarity.csv',delimiter=',')
