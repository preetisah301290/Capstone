from util import *
from constants import taoteching_input_file, pickle_path, csv_path,BookWisdom_file, BookWisdom_studyline
import pickle
from collections import Counter

def BookWisdom_preprocess():
    ...
    of = open(pickle_path+"BookWisdom.pickle", "wb")
    pickle.dump(final_dict, of)


def BookWisdom_baseline():
    f = open(pickle_path+"BookWisdom.pickle","rb")
    ...
    execute_similatity_matrix(DTM,  type="BookWisdom", label="baseline", col_row_labels=row_labels)

    of = open(pickle_path+"BookWisdom.pickle","wb")
    pickle.dump(input_dict, of)

def BookWisdom_find_similarChp(Chapter_index):
    data = np.genfromtxt(csv_path+'BookWisdom_baselineCosineSimilarity.csv',delimiter=',')
